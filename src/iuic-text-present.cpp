...
void present::aggregator::apply_present(size_t id, glyph::sequence present) {
  if (presents.contains(id)) {
    presents.at(id).glyphs = present;
  }
};

token::sequence present::aggregator::get_tokens(size_t id) const {
  if (presents.contains(id)) {
...
glyph::sequence present::aggregator::get_present(size_t id) const {
  if (presents.contains(id)) {
